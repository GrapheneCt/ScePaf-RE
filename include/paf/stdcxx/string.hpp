...
namespace ccc {
	...
	enum Flag
	{
		Flag_AllowIllegalCode = 0x2,
		Flag_AllowOutOfCodeRange = 0x4,
		Flag_AllowInvalidEncode = 0x8,
		Flag_AllowSrcBufferEnd = 0x10
	};

	extern string *UTF16toUTF8WithAlloc(const wchar_t *src, string *dst = SCE_NULL);

	extern SceSSize UTF8toUTF16(const char *src, wstring *dst);

	extern SceSSize UTF8toUTF16(const char *src, wstring *dst, SceUInt32 flags);

	extern SceSSize UTF8toUTF16(string *src, wstring *dst);

	extern SceSSize UTF8toUTF16(string *src, wstring *dst, SceUInt32 flags);

	extern SceSSize UTF16toUTF8(const wchar_t *src, string *dst);

	extern SceSSize UTF16toUTF8(const wchar_t *src, string *dst, SceUInt32 flags);

	extern wstring *UTF8toUTF16WithAlloc(const char *src, wstring *dst = SCE_NULL);

	extern SceSSize UTF16toUTF8(wstring *src, string *dst);

	extern SceSSize UTF16toUTF8(wstring *src, string *dst, SceUInt32 flags);

	extern string MemsizeFormat(SceSize memsize);

	extern string Sprintf(const char *format, ...);
}
